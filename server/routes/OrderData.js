const express = require('express')
const router = express.Router()
const Order = require('../models/Orders')

router.post('/placeOrder', async (req, res) => {
    let data = req.body.order_data
    // await data.splice(0,0,{Order_date: req.body.order_date})

    let eId = await Order.findOne({"email": req.body.email})
    console.log(eId)

    if (!eId) {
        try {
            await Order.create({
                email: req.body.email,
                order_data: [data]
            }).then(() => {
                res.json({ success: true, message: 'Order placed successfully' , orderData: [data]})
            })
        } catch (error) {
            res.json({ error: error, message: error.message })
        }
    }
    else {
        try {
            await Order.findOneAndUpdate({email: req.body.email}, { $push: { order_data: data } }).then(() => {
                res.json({ success: true, message: 'Order updated successfully', orderData: [data[1]] })
            })
        } catch (error) {
            res.json({error: error, message: error.message})
        }
    }
})


router.post("/myOrder", async (req, res) => {
    try {
        let myOrder = await Order.findOne({"email": req.body.email})
        res.json({myOrders: myOrder})
    } catch (error) {
        res.json({error: error, message: error.message}) 
    }
})

module.exports = router;